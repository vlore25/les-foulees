...
import { searchAllUsersByName } from "../../users/user.action";
...
interface SearchAllUsersProps {
    onSelectUser?: (user: UserSearchResult | null) => void;
}

export function SearchAllUsers({ onSelectUser }: SearchAllUsersProps) {
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(async () => {
            if (query.length >= 2) {
                setLoading(true);
                try {
                    const data = await searchAllUsersByName(query);
                    setResults(data);
                    setOpen(true);
                } catch (error) {
